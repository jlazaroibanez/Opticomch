...
def comProductivity(fnet, D = 0.1, glc = 5.0, X0 = 0.5, Xf = 5.0, XNsamps = 4, Xint = 0.2):
    # Computes the maximum productivity and the optimal cell densisty for which it is obtained
    # for a given dilution rate D and glucose concentration glc
    ...
    for X in np.linspace(X0, Xf, XNsamps):
        ...
        try:
            ...
            print("  Productivity with X in [", f"{X:.2f}",",", f"{X+Xint:.2f}", "]:", f"{prod:.4f}")            
            if (prod > maxProd):
                maxProd = prod
                optX = netobj.places['X'].avm # Cell density for which the productivity is maximum
        except:
            print("  Problem infeasible with X in [", f"{X:.2f}",",", f"{X+Xint:.2f}", "]")
